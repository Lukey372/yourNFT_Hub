...
let SOLANA_CLUSTER_ENDPOINT = CLUSTER_API;//useMemo(() => clusterApiUrl(network), [network]);
(async () => {
  try {
    ...
    if (cluster?.value) {
      SOLANA_CLUSTER_ENDPOINT = cluster?.value;
    }
  }
  catch {
...
console.log(`SOLANA_CLUSTER_ENDPOINT`, SOLANA_CLUSTER_ENDPOINT);
...
export class SolanaClient {
  ...
  constructor(props?: SolanaClientProps) {
    this.endpoint = props?.rpcEndpoint ? props?.rpcEndpoint : this.endpoint;
    try {
